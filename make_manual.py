...
pdf.cell(0, 6, "Версия: август 2026 (v2.3)", new_x="LMARGIN", new_y="NEXT", align="C")
...
"- Импорт меток из открытой карточки админки (кнопка «Импорт»)\n"
"- Поддержка Chrome, Edge, Яндекс.Браузера, Opera и Firefox\n"
...
"- «Импорт» — собрать метки из открытой карточки админки в программу\n"
"  (мидролы, заставки, предыдущие, построл, длительность). Работает через "
"браузерное расширение: расширение читает поля формы и отправляет данные "
"обратно в программу")
pdf.ln(4)
...
pdf.add_page()
pdf.set_font("Segoe", "B", 16)
pdf.cell(0, 10, "6. Браузерное расширение", new_x="LMARGIN", new_y="NEXT")
pdf.ln(2)
pdf.set_font("Segoe", "", 10)

pdf.multi_cell(0, 5,
"Расширение «IVI Fill» (manifest v3) — папка ivi_ext/.\n"
"Работает в Chrome, Edge, Яндекс.Браузере, Opera и Firefox.\n\n"
...
pdf.set_font("Segoe", "B", 12)
pdf.cell(0, 8, "Импорт из карточки", new_x="LMARGIN", new_y="NEXT")
pdf.set_font("Segoe", "", 10)
pdf.multi_cell(0, 5,
"Кнопка «Импорт» в панели меток собирает данные из открытой карточки "
"админки в обратном направлении:\n"
"1. Программа отправляет номер запроса (GET /import_request)\n"
"2. Расширение (раз в 2 сек) замечает запрос, читает поля формы:\n"
"   midroll-*-time, localization_labels (заставки/предыдущие),\n"
"   id_credits_begin_time (построл), id_duration (длительность)\n"
"3. Данные отправляются POST /import и применяются в панели меток\n"
"4. Если карточка не открыта — программа сообщит об ошибке\n\n"
"Все значения очищаются от пробелов-разделителей тысяч\n"
"(поля админки класса thousand_separator): «2 550» = 2550 секунд.")
pdf.ln(4)
...
"    GET /import_request — номер текущего запроса импорта\n"
"    POST /import — данные из карточки, присланные расширением (JSON)\n\n"
"Поток данных импорта:\n"
"1. Кнопка «Импорт» → /import_request (растёт номер запроса)\n"
"2. Расширение опрашивает /import_request раз в 2 сек\n"
"3. Расширение читает поля карточки и шлёт POST /import\n"
"4. _poll_sync (каждые 200 мс) применяет данные в панель меток\n\n"
"Файлы данных:\n"
...
"   - Ярлыки запуска браузера с расширением: Chrome, Яндекс, Opera, Firefox\n"
...
"Расширение браузера:\n"
"- Chrome/Edge/Opera: Chrome 137+ отключил --load-extension, поэтому используется "
"ярлык с расширением. Открывает браузер с отдельным профилем и автоматически "
"загружает расширение через CDP-pipe (рабочий профиль не затрагивается).\n"
...
"- Firefox: расширение загружается через web-ext (`npx --yes web-ext run`).\n"
"  Нужен установленный Node.js/npm (скачает web-ext при первом запуске).\n"
